...
const [bom, acceptance, pinPlan, domesticProcurement, pendantManifest] = await Promise.all([
  ...
  readFile(path.join(evtRoot, 'nc01-taobao-bom-v1.json'), 'utf8').then(JSON.parse),
...
if (process.argv.includes('--json')) {
  ...
  const domestic = report.domesticProcurement;
  console.log(`淘宝单台验证包：先买 ${domestic.buyNow} 类，器件目标 CNY ${domestic.hardwareMinCny}-${domestic.hardwareMaxCny}，含预估运费 CNY ${domestic.checkoutMinCny}-${domestic.checkoutMaxCny}`);
  console.log(`- 暂停 ${domestic.hold} 类；已下单 ${domestic.ordered} 类，已到手 ${domestic.available} 类`);
