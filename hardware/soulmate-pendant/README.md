...
| `stl/nexora-core-light-guide.stl` | 透明或半透明 PETG | 四段状态灯导光环 |
| `nexora-core-nc01-kit.3mf` | 多材料打印盘 | 三个零件已排版的整套模型 |
| `source/soulmate-pendant.scad` | - | 可修改参数的 OpenSCAD 源文件 |
| `nexora-core-nc01-print-pack.zip` | - | 交给打印用户或供应商的完整压缩包 |
...
- 三个打印件的 PLA 估算总重量约 `24.2 g`，不含电子件、螺丝和链条。
...
- 后壳背面朝下打印；前框导光槽朝下；导光环平放打印。
- 后壳和前框建议 PETG，首轮尺寸验证也可使用 PLA。
- 导光环使用透明 PETG，`100%` 填充；打印后用 `600-1000` 目砂纸轻磨以增强漫反射。
- 三个零件按当前导出方向不需要主体支撑；USB-C 短桥可开启桥接优化。
- 先打印前框和导光环，确认卡槽公差后再打印完整后壳。

## 装配顺序

1. 将导光环从前框正面压入八边形灯槽，确认四段灯带没有翘起。
