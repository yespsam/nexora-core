...
没有开发板时，可以在同一台电脑里同时运行手机网关和 NC-01 圆屏模拟器：

```text
http://localhost:8888/device-lab/
```

点击“运行全部测试”会检查双端加载、240×240 圆屏、虚拟 BLE、身份与成长同步、倾听/思考/说话状态、低电量/充电/休眠、连续对话、真实声线接口和断连显示。虚拟设备写入使用与固件相同的 BLE JSON 字段。

完整代码、固件和数字样机测试可在仓库根目录一次运行：

```bash
npm run test:computer
```

详细范围和验收方法见 [电脑测试说明](docs/COMPUTER_TESTING.md)。电池温升、真实续航、麦克风回声、扬声器音质、蓝牙距离和佩戴强度仍必须等待实体样机。
